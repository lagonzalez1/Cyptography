...
/*
 * DES Completed K1 K2 can be generated
 * S-DES: To-do
 *
 * 
 * 
...
public class Lab {
	...
	static byte[] rawkey = {1,0,1,0,0,0,0,0,1,0};
	public static Keys keys = new Keys(rawkey);
	...
	public static void main(String[] args) {
		...
		byte[] encrypted = Encrypt(rawkey, plaintext);
	...
	public static byte[] getP4(byte[] left, byte[] right) {
		...
		System.out.println(searchMatrix(s0,leftRow, leftCol));
		System.out.println(searchMatrix(s1,rightRow, rightCol));
	...
	public static byte[] Encrypt(byte[] rawkey, byte[] plaintext) {
		byte[] bit = new byte[8]; // Must be of size 4
		...
		// right side will be used S1
		// left side will be used S0
		
		// P4 given here. 
		
		byte[] p4 = functionK(right);
		// END
		
		// XOR P4 with left side of IP
		byte[] FKleft = XOR(left, p4);
		//END
		
		
		// Cross to complete
		bit = combineBytes(right, FKleft);
	...
	public static byte[] functionK(byte[] right) {
		...
		byte[] key1 = keys.getKey1();
		byte[] key2 = keys.getKey2();
